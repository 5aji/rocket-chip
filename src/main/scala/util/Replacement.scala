...
//  }
  ...
  def get_next_state(state: UInt, touch_way: UInt): UInt = {
    ...
    // TODO: nbits-1 or it breaks compilation
    val prev_group_state = state(nBits - 1, nBits - (nGroups * log2Ceil(n_states_group))).asTypeOf(next_group_state)
    val maxgroup_value = prev_group_state.reduceLeft((x, y) => Mux(x > y, x, y))
    val maxgroup_idx = prev_group_state.indexWhere((x: UInt) => x === maxgroup_value)
    val group_incr = n_states_group.U - 1.U - maxgroup_value // -1 again only for maxway
    next_group_state.zipWithIndex.foreach({ case (el, idx) =>
      el := Mux(idx.U === touch_way / nGroups.U,
        0.U(log2Ceil(n_states_group).W),
        Mux(idx.U === maxgroup_idx,
            prev_group_state(idx) + group_incr - 1.U,
            prev_group_state(idx) + group_incr))
    })
    // now we parse out the next_individual_state and prevstate
    val next_individual_state = Wire(Vec(n_ways, UInt(log2Ceil(n_states_individual).W)))
    // TODO: offbyone?
    val prev_individual_state = state.apply(n_ways * log2Ceil(n_states_individual) - 1,0).asTypeOf(next_individual_state)

    // we now create our maxval/maxidx vec
    val chunkmaxidx = Wire(Vec(nGroups, UInt(log2Ceil(group_width).W)))
    val chunkmaxval = Wire(Vec(nGroups, UInt(log2Ceil(n_states_individual).W)))


    // populate each one with the subsection.
    prev_individual_state.grouped(group_width).zipWithIndex.foreach(ch => {
      val groupidx: Int = ch._2
      val v = VecInit(ch._1) //TODO: is this ok?
...
//      })
    ...
    next_individual_state.zipWithIndex.foreach({ case (w, idx) =>
      val group = idx % group_width
      val subindex = idx - group
      w := Mux(idx.U === touch_way, 0.U,
        Mux(group.U === maxgroup_idx,
          Mux(subindex.U === chunkmaxidx(group), n_states_individual.U - 2.U, prev_individual_state(idx) + chunkmaxval(group)),
          prev_individual_state(idx)
      ))
    })
  ...
  def get_replace_way(state: UInt): UInt = {
    ...
    val prev_individual_state = state.apply(n_ways * log2Ceil(n_states_individual) - 1,0).asTypeOf(Vec(n_ways, UInt(log2Ceil(n_states_individual).W)))
    // find group where groupval === max_group_val - 1
    val groupidx = prev_group_state.indexWhere(x => x === n_states_group.U - 2.U)
    val chunkmaxidx = Wire(Vec(nGroups, UInt(log2Ceil(group_width).W)))
    prev_individual_state.grouped(group_width).zipWithIndex.foreach(ch => {
      val groupidx: Int = ch._2
      val v = VecInit(ch._1) //TODO: is this ok?
